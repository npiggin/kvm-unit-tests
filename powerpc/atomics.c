...
static void test_lwarx_stwcx(int argc, char *argv[])
{
	...
	*var = 0;
	asm volatile (
		      "lwarx	%0,0,%1;"
		      : "+&r"(old) : "r"((char *)var + 1));
	report(old == 10 && got_interrupt && recorded_regs.trap == 0x600, "unaligned lwarx");
	got_interrupt = false;
	...
	*var = 0;
	...
	report(old == 0 && *var == 0 && got_interrupt && recorded_regs.trap == 0x600, "unaligned stwcx.");
	got_interrupt = false;
